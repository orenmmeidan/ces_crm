...
class AdminUsers extends Query{
	 ...
	 public  function isLoggedIn(){
        ...
        if(isset($_SESSION['user_id']) && isset($_SESSION['remote']) &&
...
//-----------------------------------------------------------------------------------------------------------
	public function DisplayUsers(){
		...
		$table_titles_arr = array("First Name", "Last Name" ,"Username","Email","Position");
...
//-----------------------------------------------------------------------------------------------------------
	public function ProcessUser($user_arr){
			if($user_arr['submit']){
			//print_r($user_arr);	
			if($this->ValidateName($user_arr['fname'])===false){$message.="<span>First Name is too short</span>";}
			if($this->ValidateName($user_arr['lname'])===false){$message.="<span>Last Name is too short</span>";}
			if($this->ValidateName($user_arr['user_name'])===false){$message.="<span>Username is too short</span>";}
			if($this->ValidateUsername($user_arr['user_name'])===TRUE){$message.="<span>Username already exists. try a different one</span>";}
			if($this->ValidateEmail($user_arr['email'])===false){$message.="<span>Email format is not valid</span>";}
			if($this->CheckPW($user_arr['password'])<3){$message.="<span>Password is too easy, use at least 4 charachters, letters and numbers.</span>";}
			
				if(!$message){
				$fields = array("fname"=>$user_arr['fname'],"lname"=>$user_arr['lname'],"user_name"=>$user_arr['user_name'],"email"=>$user_arr['email'],"password"=>md5($user_arr['password']));	
